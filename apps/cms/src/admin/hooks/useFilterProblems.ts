...
import { Problem, Source } from 'core/payload-types'
import qs from 'qs'
...
export const useFilterProblems = ({
	...
	const query = useQuery<PaginatedDocs<Problem>, ErrorResponse>({
		...
		queryFn: async () => {
			if (sourceId) {
				const sourceResponse = await fetch(`${serverURL}${api}/sources?${qs.stringify(sourceQueryParams)}`, {
					credentials: 'include',
				})
			...
			if (resultOnNoFilter === 'empty' && (!ids || ids.length === 0) && !sourceId && !tagId && !searchInput) {
				return emptyPaginatedDocs
			}

			const problemResponse = await fetch(`${serverURL}${api}/${Problems.slug}?${qs.stringify(problemQueryParams)}`, {
				credentials: 'include',
			})
